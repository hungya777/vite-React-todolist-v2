...
import { todos, deleteTodo } from "../services/todoService";

function TodoList() {
    let navigate = useNavigate();
    const token = getCookie("token");
    
    const [user] = useState(JSON.parse(atob(token.split('.')[1])));
    ...
    const removeTodos = async () => {
        const needDeletes = data.filter((item) => item.status).map(item => {
                    ...
                    (<div className="todoList_list">
                        ...
                        <div className="todoList_items">
                            ...
                            <div className="todoList_statistics">
                                <p>
                                    {data.filter((item) => !item.status).length} 個待完成項目
                                </p>
